...
class Firewall
{
    ...
    /**
     * Checks if the user is connected through a proxy server.
     *
     * @param $id string IP / Device to check.
     * @return boolean TRUE if it's behind a proxy; FALSE otherwise.
     */
    public function isBehindProxy($id = null)
    {
        if (!$id) {
            $id = $this->_id;
        }
        $res = ((isset($_SERVER['HTTP_X_FORWARDED_FOR']) && $_SERVER['HTTP_X_FORWARDED_FOR'])
